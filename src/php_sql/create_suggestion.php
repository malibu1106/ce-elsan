...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO suggestions (company_name, description, image_url, address, user_id) 
            VALUES (:company_name, :description, :image_url, :address, :user_id)";
    
    $stmt = $db->prepare($sql);
    $stmt->execute([
        ':company_name' => $company_name,
        ':description' => $description,
        ':image_url' => $target_file, // Chemin de l'image dans le serveur
        ':address' => $address,
        ':user_id' => $user_id
    ]);
